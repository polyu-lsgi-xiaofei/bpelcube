...
create table BPEL_EVENT (ID bigint not null, IID bigint, PID bigint, TSTAMP timestamp, TYPE varchar(255), DETAIL longvarchar, DATA blob(2G), SID bigint, INSERT_TIME timestamp, MLOCK integer not null, primary key (ID));
...
create table BPEL_XML_DATA (ID bigint not null, DATA blob(2G), NAME varchar(255) not null, SIMPLE_VALUE varchar(255), SCOPE_ID bigint, PIID bigint, IS_SIMPLE_TYPE bit, INSERT_TIME timestamp, MLOCK integer not null, primary key (ID));
...
create table dual_hibernate_seqhilo (zero integer);
insert into dual_hibernate_seqhilo values (0);
create sequence hibernate_seqhilo start with 1 increment by 1;
